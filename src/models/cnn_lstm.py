...
class CNNLSTMModel(nn.Module):
    def __init__(self, eeg_channels, latent_dim=64, num_classes=2):
        ...
        # Replace Swin with LSTM
        self.lstm = nn.LSTM(input_size=32, hidden_size=64, num_layers=2, batch_first=True, bidirectional=False)
    ...
    def forward(self, x_eeg_seq, x_latent_seq, return_features=False, xai_mode=False):
        ...
        x, _ = self.lstm(x)                  # (B*S, T', 64)
        
        x_post_lstm = x.transpose(1, 2)                # (B*S, 64, T')
        window_features = self.pool(x_post_lstm).squeeze(-1) 
